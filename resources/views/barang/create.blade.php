        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative overflow-x-auto p-5">
                    ...
                    <form method="post" action="{{ route('barang.store') }}" class="max-w-md mx-auto">
                        @csrf
                        <div class="relative z-0 w-full mb-5 group">
                            <input type="text" name="nama" id="floating_name"
                                ...
                                placeholder=" " autocomplete="off" value="{{ old('nama') }}" />
                            ...
                            <select id="kategori" name="kategori"
                                ...
                                @foreach ($kategoris as $kategori)
                                    <option value="{{ $kategori->name }}"
                                        {{ old('kategori') == $kategori->name ? 'selected' : '' }}>{{ $kategori->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="relative max-w-sm mb-5">
                            <label for="datePickerId"
                                class="block mb-2 text-sm text-gray-900 dark:text-slate-400">Tanggal Masuk</label>
                            <input id="datePickerId" type="date" name="tanggal_masuk"
