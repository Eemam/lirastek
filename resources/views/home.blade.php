                                    ...
                                    <tbody>
                                        @foreach ($items as $index => $item)
                                            <tr class="border-b border-neutral-200 dark:border-white/10 text-center">
                                                ...
                                                @auth
                                                    <td class="whitespace-nowrap px-6 py-4">
                                                        <a href="{{ route('barang.edit', $item->nama) }}"
                                                            class="px-3 py-1.5 rounded-lg bg-green-300 hover:bg-green-500 hover:text-white">Edit</a>
                                                        <form action="{{ route('barang.destroy', $item->nama) }}"
                                                            class="inline" method="post">
                                                            @csrf
                                                            @method('delete')
                                                            <button type="submit"
                                                                class="px-3 py-1.5 rounded-lg bg-red-300 hover:bg-red-500 hover:text-white"
                                                                onclick="return confirm('Hapus {{ $item->nama }} dari daftar barang?')">Delete</button>
                                                        </form>
                                                    </td>
                                                @endauth
                                            </tr>
